// generated by build.php at 2020-09-10 15:53:28

use async_std::sync::Arc;
use core::operator::{http_api, http_server, sql, sql_runner, wasmtime, Monad, Source};
...
async fn main() {
    ...
    let config4 = serde_json::from_str("{\"listen_addr\":\"127.0.0.1:8099\"}").unwrap();

    let mut op4 = Arc::new(http_server::HTTPServer::new(config4));

    let config5 = serde_json::from_str("{\"wat\":{\"FilePath\":\"/Users/xuzhiqiang/Desktop/workspace/opensource/rust_exp/hi/target/wasm32-unknown-unknown/release/hi.wasm\"}}").unwrap();

    let op5 = Arc::new(wasmtime::WasmTime::new(config5));
    ...
    Arc::get_mut(&mut op4).unwrap().apply(Arc::downgrade(&op2));
